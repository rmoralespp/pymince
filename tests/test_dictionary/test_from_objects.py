...
def test_dict_from_objects_with_func_getter():
    values = iter([1, 2, 3])
    result = pymince.dictionary.from_objects(values, lambda n: n, lambda n: n**2)
    assert result == {1: 1, 2: 4, 3: 9}


def test_dict_from_objects_with_itemgetter_index():
    keygetter = operator.itemgetter(0)
    valgetter = operator.itemgetter(1, 2)
    values = iter([(1, "a", "b"), (2, "a", "b")])
    result = pymince.dictionary.from_objects(values, keygetter, valgetter)
    assert result == {1: ("a", "b"), 2: ("a", "b")}


def test_dict_from_objects_with_itemgetter_name():
...
def test_dict_from_objects_with_attrgetter_name():
...
def test_dict_from_objects_with_duplicated_key():
    keygetter = operator.itemgetter(0)
    valgetter = operator.itemgetter(1)
    values = iter([["a", 1], ["a", 2], ["b", 3]])

    with pytest.raises(ValueError):
        pymince.dictionary.from_objects(values, keygetter, valgetter)
