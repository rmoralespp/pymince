...
def test_replace_one():
    ...
    result = pymince.iterator.replace(values, matcher, "foo", count=1)
...
def test_replace_two():
    values = (1, 1, 1)
    matcher = lambda n: n == 1
    result = pymince.iterator.replace(values, matcher, "foo", count=2)
...
def test_replace_undefined():
    values = (1,) * 100
    matcher = lambda n: n == 1
    result = pymince.iterator.replace(values, matcher, "foo")
